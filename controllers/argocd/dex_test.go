...
func TestReconcileDex_WithLabels(t *testing.T) {
	logf.SetLogger(ZapLogger(true))
...
func TestReconcileDex_WithAnnotations(t *testing.T) {
	logf.SetLogger(ZapLogger(true))
	a := makeTestArgoCD(func(a *argoproj.ArgoCD) {
		a.Spec.SSO = &argoproj.ArgoCDSSOSpec{
			Provider: argoproj.SSOProviderTypeDex,
...
func TestReconcileDex_WithLabelsAndAnnotations(t *testing.T) {
	logf.SetLogger(ZapLogger(true))
	a := makeTestArgoCD(func(a *argoproj.ArgoCD) {
		a.Spec.SSO = &argoproj.ArgoCDSSOSpec{
			Provider: argoproj.SSOProviderTypeDex,
	...
	// Verify custom labels and annotations are applied to pod template
	assert.Equal(t, "custom-value", deployment.Spec.Template.Labels["custom-label"])
	assert.Equal(t, "custom-value", deployment.Spec.Template.Annotations["custom-annotation"])
}
